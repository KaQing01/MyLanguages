...
function Vocabulary() {
  ...
  const handleDelete = async (id) => {
    const res = await fetch(`http://localhost:5050/api/words/${id}`, {
      method: 'DELETE',
      credentials: 'include',
    });
    if (res.ok) {
  ...
  const handleEditSubmit = async (e) => {
    e.preventDefault();
    const res = await fetch(`http://localhost:5050/api/words/${editingId}`, {
      method: 'PATCH',
      credentials: 'include',
  ...
  const toggleFavorite = async (word) => {
    ...
    const res = await fetch(`http://localhost:5050/api/words/${word._id}`, {
      method: 'PATCH',
      credentials: 'include',
      headers: { 'Content-Type': 'application/json' },
